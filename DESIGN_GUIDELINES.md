...
- Shared primitives are closer to the intended baseline, but a concentrated band of admin and business-setup UI still relies on bespoke color tokens, direct hex values, and local surface styling instead of the shared theme vocabulary.
  Affected frontend areas: `apps/frontend/src/pages/admin/users/AdminUserDetailsPage.tsx`, `apps/frontend/src/pages/admin/users/AdminUsersPage.tsx`, `apps/frontend/src/pages/admin/layout/AdminLayout.tsx`, `apps/frontend/src/design-system/organisms/BusinessDetailsFormPanes.tsx`, `apps/frontend/src/design-system/organisms/BusinessLicensePane.tsx`, `apps/frontend/src/design-system/organisms/BusinessLocationsPane.tsx`, and related business-management surfaces. The core `Card`, `Button`, `Input`, and `Select` primitives are no longer the main source of this gap.
- Some shell-level sections still mix live operational data with scaffold or reserved dashboard content. Placeholder structure is acceptable temporarily, but it must remain clearly non-authoritative and must not read like a finished dashboard.
  Affected frontend areas: `apps/frontend/src/pages/shell/AppHomePage.tsx`, especially the landing dashboard sections that still fall back to scaffold metrics and reserved shell blocks.
- Some async screens now handle loading and error states well, but the product is still inconsistent about distinguishing loading, empty, and loaded states in overview and maintenance surfaces.
  Affected frontend areas: overview and management pages that render sparse or empty cards before data arrives, especially `apps/frontend/src/pages/finance/OverviewPage.tsx`, `apps/frontend/src/pages/finance/AccountsPage.tsx`, and detail pages that still fall back to generic empty placeholders such as `apps/frontend/src/pages/admin/users/AdminUserDetailsPage.tsx`.
- Some screens still bypass shared design-system composition with ad hoc classes, local button treatments, or one-off control variants instead of reusing shared atoms and composed patterns consistently.
  Affected frontend areas: admin/business-management surfaces, customer-group management, category/collection management, and parts of the sales document workspace where local color overrides and one-off action treatments still appear.
- Some page-level filters and compact controls still rely on placeholder text without visible labels. This remains non-conformant even when the screen is dense.
  Affected frontend areas: filter rows such as `apps/frontend/src/pages/admin/users/AdminUsersPage.tsx`, plus compact search and filter controls in catalog, parties, and similar operational list screens that still use placeholder-only meaning.
- Some shared and non-operational screens still use roomier spacing and larger type than the default operational density described above. Treat those as exceptions to reduce over time unless intentionally documented.
  Affected frontend areas: auth and transition surfaces, especially `apps/frontend/src/design-system/organisms/LoginCard.tsx`, `apps/frontend/src/pages/auth/LoginPage.tsx`, `apps/frontend/src/pages/system/OfflinePage.tsx`, `apps/frontend/src/pages/system/SessionSplashPage.tsx`, the generic `.page` utility in `apps/frontend/src/styles.css`, and parts of the shell landing layout in `apps/frontend/src/pages/shell/AppHomePage.tsx`.
- The mobile fixed action-bar pattern remains useful, but it can still separate save/cancel controls from the form section they affect. Keep using it only where the mobile locality tradeoff is clearly worth it.
  Affected frontend areas: pages that use `apps/frontend/src/design-system/molecules/PageActionBar.tsx`, especially item creation/editing, customer-group management, and admin detail screens.
